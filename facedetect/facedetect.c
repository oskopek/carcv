...
void detect_and_draw( IplImage* img, double scale_factor, int min_neighbors, int flags, CvSize min_size, CvSize max_size );
...
int main( int argc, char** argv )
{
    ...
    CvSize max_size = cvSize(2000,2000);
    ...
    if( capture )
    {
        for(;;)
        {
            ...
            if( !frame_copy )
                ...
                detect_and_draw( frame_copy, scale_factor, min_neighbors, flags, min_size, max_size );
    ...
    else
    {
        ...
        if( image )
        {
            detect_and_draw( image, scale_factor, min_neighbors, flags, min_size, max_size );
            cvWaitKey(0);
